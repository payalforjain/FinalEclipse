...
public class CheckIfTreeIsBst {
    ...
    public static boolean isBstUsingRecursion(BinarySearchTree searchTree) {
        BSTNode node = searchTree.getRootOfTree();
        if (node == null) {
            return false;
        }
    ...
    private static void isBstUsingRecursion(BSTNode node) {
        if (node != null) {
            isBstUsingRecursion((BSTNode) node.getLeftChild());
            if (isBst) {
                inOrderList.add(node.getData());
                checkOrder();
                isBstUsingRecursion((BSTNode) node.getRightChild());
            }
        }
    }
    ...
    private static void isBstNonRecursive(BSTNode node) {
        Stack<BSTNode> stack = new Stack<BSTNode>();
        stack.push(node);
        while (!stack.isEmpty() && isBst) {
            while (node.getLeftChild() != null && !node.isVisited()) {
                node = (BSTNode) node.getLeftChild();
                stack.push(node);
            }
            node = stack.pop();
            ...
            if (node.getRightChild() != null) {
                node = (BSTNode) node.getRightChild();
                stack.push(node);
            }
        }
    }
